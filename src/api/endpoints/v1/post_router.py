...
async def get_post(
    slug: str,
    request: Request,
    current_user: Any = Depends(AuthService.get_optional_current_user),
    ...
    await post_service.increment_post_view(
        post_id=post["id"],
        user_id=user_id,
...
@post_router.post("/{post_id}/like" , status_code=status.HTTP_200_OK,summary="Toggle like for a post")
async def toggle_post_like(
    ...
    result=await post_service.manage_post_like(post_id=post_id, user_id=user_id)
    if result.get("status") == "error":
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=result.get("message", "خطایی رخ داده است")
        )
    return result
