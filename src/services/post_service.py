...
class PostService:
    ...
    async def increment_post_view(self , post_id:int , user_id:Optional[int] , ip_address:str) ->bool:
        is_unique_view=self.post_repository.add_view_if_unique(
            post_id=post_id,
            user_id=user_id,
            ip_address=ip_address
        )
        if is_unique_view is True:
            print(f"👁️ بازدید یکتای جدید برای پست {post_id} از آی‌پی {ip_address} ثبت شد.")
        return is_unique_view

    async def manage_post_like(self , post_id:int , user_id:int):
        post=self.post_repository.get_by_id(post_id)
        if post is None:
            return {"status":"error" ,"message":"پست یافت نشد"}
        result=self.post_repository.toggle_like(post_id=post_id, user_id=user_id)
        if result["status"]=="liked" and result["likes_count"]%100==0 and result["likes_count"]>0:
            author_id=post.get("user_id")
            self.user_repository.increase_reputation(user_id=author_id,reward_score=1 )
            print(f"🎉 تبریک! پست {post_id} به {result['likes_count']} لایک رسید. ۱ امتیاز به کاربر {author_id} داده شد.")
