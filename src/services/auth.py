...
from src.config import settings
from src.repositories.user_repository import UserRepository
# خط مربوط به user_service حذف شد تا دایره شکسته بشه

pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")
# oauth2_scheme = OAuth2PasswordBearer(tokenUrl="api/v1/users/login") # آدرس رو با روتور جدیدت ست کن
...
class AuthService:
    ...
    @staticmethod
    def get_password_hash(password: str):
        # محدودیت bcrypt برای رمزهای طولانی (بیشتر از ۷۲ بایت)
        password_bytes = password.encode('utf-8')
        if len(password_bytes) > 72:
            password = password_bytes[:72].decode('utf-8', errors='ignore')
        return pwd_context.hash(password)
    ...
    async def get_current_user(token: str = Depends(oauth2_scheme), user_repo: UserRepository = Depends()):
        ...
        # نکته: اگر متد get_by_id در ریپازیتوری async هست، اینجا await بذار
        user = user_repo.get_by_id(int(user_id))
